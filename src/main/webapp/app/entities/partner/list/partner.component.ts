...
import { EntityArrayResponseType, PartnerService } from '../service/partner.service';
...
export class PartnerComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPartner): number => this.partnerService.getPartnerIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.partners = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPartner[] | null): IPartner[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.partnerService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
